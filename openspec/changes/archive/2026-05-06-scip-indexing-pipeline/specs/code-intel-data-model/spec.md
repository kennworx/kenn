> **Note (post-source-data-model):** The schema-level requirements in this capability
> (symbol identity, path canonicalization, edge derivation, kind enum, etc.) are now
> defined authoritatively in the `source-data-model` proposal. Requirements below that
> reference standalone types (`SymbolKind`, `EdgeKind`, `OccurrenceRecord`,
> `SymbolId`) are read against `source-data-model`'s `Kind`, `EdgeKind`, in-memory
> occurrence intermediate, and `PublicId` respectively. The producer-specific
> requirements (path canonicalization, edge derivation, run-report counters, the
> `Sink` trait) remain in force.
