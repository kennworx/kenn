...
SHALL NOT read, stat, or open any written file — its only side effect is the
SQLite write (and, on error, a diagnostic log append).
...
`UserPromptSubmit`, `PreToolUse` (matcher `Bash`), `PostToolUse` (matcher
`Bash`), `PostToolUse` (matcher `Edit|Write`), and `SessionEnd` to the
corresponding `kenn cc-hook` subsubcommands. The subcommand SHALL accept an
...
store SHALL NOT contain file sizes, file-existence confirmation, or edit-body
...
**Reason:** The `ready/<session_id>` marker existed only for a never-built JSONL
ingest pass. With direct SQLite writes there is no inbox to enumerate, so the
marker has no consumer.

**Migration:** None. `session-end` now sets `ended_at` on the `sessions` row;
existing `local/history/ready/` markers are abandoned and may be deleted out of
band.

### Requirement: the raw record schema is a tagged-union JSON line

**Reason:** The per-session JSONL inbox (`local/history/<session_id>.jsonl`) and
its tagged-union line schema are replaced by the relational SQLite schema
(`sessions → commands → files`). Capture is no longer append-a-JSON-line.

**Migration:** None. The JSONL inbox was write-only with no consumer; existing
`*.jsonl` files are abandoned and may be deleted out of band.
