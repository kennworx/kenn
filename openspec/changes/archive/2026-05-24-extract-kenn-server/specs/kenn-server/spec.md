...
#### Scenario: bind conflict surfaces cleanly
...
to a file at the per-OS state directory resolved via the
`directories` crate: `~/.local/state/kenn/server.pid` on Linux,
`~/Library/Application Support/kenn/server.pid` on macOS,
