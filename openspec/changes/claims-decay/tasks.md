...
- [ ] 3.1 Mark verification status on claims returned by `kenn findings directives` (and
...
- [ ] 4.1 Add a verification outcome to `kenn findings touch` — still true, no longer
  true, partially true — separate from `attach`. → verify: the outcome is recorded and
  readable.
- [ ] 4.2 Do NOT let `attach` clear the unverified mark. `attach` means "this applied to
...
- [ ] 4.3 Record "no longer true" by superseding with a finding describing the current
...
- [ ] 4.4 Express a partial fix as a distinct outcome. The motivating failure was a
...
- [ ] 5.1 Add a claims step to the `kenn:squeeze` skill: for the changed paths, re-verify
...
- [ ] 5.2 Say plainly in the skill that a drifted rule usually still holds and a drifted
  claim may not, so the two are not worked through with the same effort. → verify: the
  guidance distinguishes them.

## 6. Verification

- [ ] 6.1 Replay both motivating incidents against the implementation: the zero-range-def
