...
On an index run, every code row that `code-search` reconciliation marks for re-embedding SHALL be embedded via the producer and have its `embedding` column populated; unchanged rows SHALL reuse their committed embedding. On a findings flush, every newly committed finding SHALL be embedded via the producer.
