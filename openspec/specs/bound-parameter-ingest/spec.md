# bound-parameter-ingest

## Purpose

Defines the contract that the rust-side `SurrealdbSink` writes records via parameter-bound SurrealQL queries instead of inline-array literals. Each non-empty record table per batch binds to a named parameter and is referenced by name in the query string, so SurrealDB skips per-row SurrealQL parsing. The streaming JSONL ingest path is also threaded so the producer is not stalled by sink-flush latency.
