# oci-image-build Specification

## Purpose

The six indexer images are built by `docker buildx bake` from per-language
Dockerfiles committed alongside them, all on a single glibc base
(`ubuntu:noble`). Each image carries only its payload — the indexer binary, the
auxiliary executables it spawns at index time (for example `git` for the
TypeScript and Python indexers), the shared libraries they link, a CA
certificate bundle, and the shared kenn-toolchain provisioning entrypoint — and
no bundled language toolchain, because the toolchain version belongs to the
workspace and is provisioned at run time into the shared cache. Every step that
can happen in the build happens in a Dockerfile stage (binaries are compiled in a
builder stage for the image's own platform, not cross-compiled on the host), so
the result depends only on Docker being present, and an image is accepted only
after the published artifact is pulled and used to index a real fixture — not on
a passing `--version`.
